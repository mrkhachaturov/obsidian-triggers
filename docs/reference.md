<!-- Generated by scripts/generate-docs.mjs. Edit docs/templates/reference.md. -->
...
| --- | --- | --- | --- |
| **Path** | every view | **is**, **contains**, **does not contain**, **starts with**, **ends with**, **in folder**, **not in folder** | `Enter a value` |
| **Base view** | `bases` | **is**, **contains**, **does not contain**, **starts with**, **ends with** | `Enter a view name` |
| **Property** | every view | **is**, **contains**, **does not contain**, **starts with**, **ends with**, **exists**, **is missing**, **is empty** | `Enter a value` |

| Operator | Holds when |
| --- | --- |
| **is** | the value is exactly this |
| **contains** | the value has this inside it |
| **does not contain** | it does not |
| **starts with** | the value begins with this |
| **ends with** | the value ends with this |
| **in folder** | the file sits under this folder, at any depth |
| **not in folder** | it does not |
| **exists** | the property is present, whatever it holds |
| **is missing** | the property is not present |
| **is empty** | the property is present and holds nothing |
...
| Variable | Value |
| --- | --- |
| `trigger.viewType` | `markdown`, `bases`, `canvas`, … |
| `trigger.path` | vault path of the file in view, empty when there is none |
| `trigger.viewName` | the Base's current view, empty elsewhere |
| `trigger.rule` | the name of the rule that fired |
| `trigger.fm.<key>` | one per frontmatter property, as text |

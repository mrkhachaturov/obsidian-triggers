...
export class ContextWatcher implements IContextReader {
  ...
  start(): void {
    ...
    this.plugin.registerEvent(
      workspace.on('active-leaf-change', (leaf) => this.publish(readContext(leaf))),
    );
    ...
    this.plugin.registerEvent(
      workspace.on('layout-change', () => this.publish(readContext(workspace.getMostRecentLeaf()))),
    );

    workspace.onLayoutReady(() => this.publish(readContext(workspace.getMostRecentLeaf())));
  }
